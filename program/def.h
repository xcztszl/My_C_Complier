...
//以下语法树结点类型、三地址结点类型等定义
struct opn{
    int kind;    //标识联合成员的属性
    int type;    //标识操作数的数据类型
    union {
        int     const_int;      //整常数值
        float   const_float;    //浮点常数值
        char    const_char;     //字符常数值
...
struct symbol {       //这里只列出了一个符号表项的部分属性
    char name[33];   //变量或函数名
    int level;        //层号
...
struct symbol_scope_begin {
    //当前作用域的符号在符号表的起始位置,这是一个栈结构,当使用顺序表作为符号表时，进入、退出一个作用域时需要对其操作，以完成符号表的管理。对其它形式的符号表，不一定需要此数据结构
    int TX[30];
    int top;
    } symbol_scope_TX;
